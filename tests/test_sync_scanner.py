...
def test_scanner_context_manager(fake_bridge):
...
def test_scanner_capabilities(fake_bridge):
    device = ScanDevice(driver=Driver.WIA, id="dev-1", name="Scanner")
...
def test_scanner_scan(fake_bridge, sample_image):
...
def test_scanner_stop(fake_bridge):
    device = ScanDevice(driver=Driver.WIA, id="dev-1", name="Scanner")
    scanner = Scanner(device)
    scanner.open()
